...
exports.SEVEN_DAYS = SEVEN_DAYS; // used by the simulators to cap durations
...
exports.stripUnwantedFields = function(record) {
  if (record == null) {
    debug('No record from previous upload');
    return null;
  }
...
exports.updatePreviousDurations = async function(data, cfg, cb) {
  ...
  try {
    ...
    if (lastBasal != null && lastBasal.deliveryType === 'suspend') {
    ...
    datum = data.post_records[index];
    if (datum != null) {
      if (lastPumpSettingsOverride != null &&
          annotate.isAnnotated(lastPumpSettingsOverride, 'tandem/pumpSettingsOverride/estimated-duration')) {
